...
public class SkegnManager {
    ...
    private Boolean writeResToExternalStorage(String resName) {
        try {
            ...
            File file = new File(getFilesDir(mContext), resName);
    ...
    public int initEngine(String appKey, String secretKey){
        ...
        try {
            ...
            sdkLogObj.put("output", getFilesDir(mContext).getAbsolutePath() + File.separator + "sdklog.txt");
            ...
            cfg.put("native", getFilesDir(mContext) + "/native.res");
            ...
            cfg.put("native_cn", getFilesDir(mContext) + "/native_cn.res");
        } catch (Exception e) {
            e.printStackTrace();
            return -1;
        }
        Log.e(TAG, "new cfg:" + cfg.toString());
    ...
    public int startSkegn(JSONObject requests, CallbackResult callbackResult)  {
        ...
        try {
            ...
            recordPath = getFilesDir(mContext).getAbsolutePath() + File.separator + "records" + File.separator + new String(id).trim() + ".pcm";
            File recordParentDir = new File(recordPath).getParentFile();
            if(!recordParentDir.exists()){
                recordParentDir.mkdirs();
            }
